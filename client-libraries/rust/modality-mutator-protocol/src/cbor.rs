use minicbor::{
    data::Tag,
    decode::{self},
    encode, Decode, Decoder, Encode, Encoder,
};
use uuid::Uuid;

use crate::attrs::{AttrVal, EventCoordinate, LogicalTime, Nanoseconds, TimelineId};
const TAG_NS: Tag = Tag::Unassigned(40000);
const TAG_LOGICAL_TIME: Tag = Tag::Unassigned(40001);
const TAG_TIMELINE_ID: Tag = Tag::Unassigned(40002);
const TAG_EVENT_COORDINATE: Tag = Tag::Unassigned(40003);

// Nanoseconds

impl Encode for Nanoseconds {
    fn encode<W: encode::Write>(&self, e: &mut Encoder<W>) -> Result<(), encode::Error<W::Error>> {
        e.tag(TAG_NS)?.u64(self.0)?;
        Ok(())
    }
}
...
impl Encode for TimelineId {
    fn encode<W: encode::Write>(&self, e: &mut Encoder<W>) -> Result<(), encode::Error<W::Error>> {
        e.tag(TAG_TIMELINE_ID)?.bytes(self.0.as_bytes())?;
        Ok(())
    }
}
...
mod test {
    ...
    fn round_trip_attr_val() {
        proptest!(|(attr_val in crate::proptest_strategies::attr_val())| {
            let mut buf = vec![];
