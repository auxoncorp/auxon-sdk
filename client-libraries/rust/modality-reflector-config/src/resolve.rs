...
) -> Result<(crate::refined::Config, AuthToken), Box<dyn std::error::Error + Send + Sync>> {
    let ConfigContext {
        config: cfg,
...
pub fn resolve_reflector_auth_token(
    ...
    if let Some(path) = cli_override_auth_token_file_path {
        ...
        if let Some(token_file_contents) =
            modality_auth_token::token_user_file::read_user_auth_token_file(&path)?
        {
    ...
    if let Ok(cwd) = std::env::current_dir() {
        ...
        if cwd_relative_path.exists() {
            if let Some(token_file_contents) =
                modality_auth_token::token_user_file::read_user_auth_token_file(&cwd_relative_path)?
            {
                return Ok(token_file_contents.auth_token);
            }
        }
    ...
    if let Some(token_file_contents) =
        modality_auth_token::token_user_file::read_user_auth_token_file(&config_relative_path)?
    {
        return Ok(token_file_contents.auth_token);
    }
